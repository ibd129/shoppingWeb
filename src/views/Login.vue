  ...
  setup () {
    ...
    function handleLogin () {
      request({
        ...
        const loginInfo = userInfo.filter(item => {
          return (item.username.indexOf(formLabelAlign.name) > -1)
        })
        if (loginInfo.length > 0 && formLabelAlign.checked === true) {
          if (loginInfo[0].password === formLabelAlign.pass) {
            formLabelAlign.isLogin = true
          }
        } else {
          ElMessage.error('请勾选我已阅读并同意网站的使用条件及隐私声明。')
        }
        if (formLabelAlign.isLogin === true) {
          ElMessage({
            message: '登录成功！',
            type: 'success'
          })
          localStorage.setItem('token', formLabelAlign.name)
          setTimeout(() => {
            router.push({
              path: '/'
            })
